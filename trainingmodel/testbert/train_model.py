...
if __name__ == "__main__":
    ...
    tokenizer = BertTokenizerFast.from_pretrained(bert)#修改分词器
    label_set = LabelSet(labels=["MASK"])
    #初始化了longformer分词器和标签集

    training = Dataset(data=training_raw, tokenizer=tokenizer, label_set=label_set, tokens_per_batch=256)#修该批次大小
    ...
    model = Model(model = "bert-base-uncased", num_labels = len(training.label_set.ids_to_label.values()))
