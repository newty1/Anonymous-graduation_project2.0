...
from Roberta_model import Model
...
if __name__ == "__main__":
    ...
    tokenizer = RobertaTokenizerFast.from_pretrained(bert)#分词器
    label_set = LabelSet(labels=["MASK"])
    #初始化了longformer分词器和标签集

    training = Dataset(data=training_raw, tokenizer=tokenizer, label_set=label_set, tokens_per_batch=256)
    dev = Dataset(data=dev_raw, tokenizer=tokenizer, label_set=label_set, tokens_per_batch=256)

    test = Dataset(data=test_raw, tokenizer=tokenizer, label_set=label_set, tokens_per_batch=256)
    ...
    model = Model(model = "roberta-base", num_labels = len(training.label_set.ids_to_label.values()))
